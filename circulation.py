...
class circulation:
        ...
        if (v1 < m and v2 < m):
            ...
            if (self.graph.has_edge(v1, v2)):
                ...
                if ([v1, v2] in self.adjacency.values()):
                    ...
                    i = list(self.adjacency.keys())[
                        list(self.adjacency.values()).index([v1, v2])]
                    ...
                    x = list(nx.common_neighbors(
                        self.circulation_graph, i, v1))
                    y = list(nx.common_neighbors(
                        self.circulation_graph, i, v2))
                    ...
                    c1 = ([r, v1] in self.adjacency.values()) or (
                        [v1, r] in self.adjacency.values())
                    # c2 is a boolean variable that holds truth value of "There is a corridor joining v2 and r"
                    c2 = ([r, v2] in self.adjacency.values()) or (
                        [v2, r] in self.adjacency.values())
                    ...
                    # Case 2: One of c1 or c2 is true - most common case
                        ...
                        # Contract v2-i
                        mod_circ = nx.contracted_edge(
                            self.circulation_graph, (v2, i), self_loops=False)
                    ...
                    # Get the corridor vertex
                    i = list(self.adjacency.keys())[
                        list(self.adjacency.values()).index([v2, v1])]
    ...
    def circulation_algorithm(self, v1: int = 1, v2: int = 2) -> int:
        ...
        final_adjacency = dict(
            zip(corridor_vertices, list(adjacency.values())))
    ...
    def adjust_RFP_to_circulation(self) -> None:
        ...
        for corridor in range(start + 1, end + 1):
            if corridor in list(self.adjacency.keys()):
                ...
                self.add_corridor_between_2_rooms(
                    self.RFP.rooms[room1], self.RFP.rooms[room2])
            else:
                continue
    ...
    def add_corridor_between_2_rooms(self, room1: Room, room2: Room) -> None:
        ...
        if (common_edge[4][1] == "N"):
            self.temp_push_states.append(
                [(room1.id, "S", "T", room2.id, "N", "B")])
        ...
        for tuple_list in self.temp_push_states:
            for each_tuple in tuple_list:
                ...
                self.calculate_edge_move(
                    self.RFP.rooms[each_tuple[0]], each_tuple[1], each_tuple[2])
                self.calculate_edge_move(
                    self.RFP.rooms[each_tuple[3]], each_tuple[4], each_tuple[5])
    ...
    def find_common_edges(self, room1: Room, room2: Room) -> tuple:
        ...
        if room1.top_left_y == room2.bottom_right_y:
            common_edge = (max(room1.top_left_x, room2.top_left_x), room1.top_left_y, min(
                room1.bottom_right_x, room2.bottom_right_x), room2.bottom_right_y, (room1.id, "N"))

        # Room1 is above Room2
        elif room1.bottom_right_y == room2.top_left_y:
            common_edge = (max(room1.top_left_x, room2.top_left_x), room2.top_left_y, min(
                room1.bottom_right_x, room2.bottom_right_x), room1.bottom_right_y, (room1.id, "S"))
        ...
        # Room1 is to right of Room2
            ...
            common_edge = (room1.top_left_x, max(room1.bottom_right_y, room2.bottom_right_y),
                           room2.bottom_right_x, min(room1.top_left_y, room2.top_left_y), (room1.id, "W"))

        # Room1 is to left of Room2
        elif room1.bottom_right_x == room2.top_left_x:
            common_edge = (room2.top_left_x, max(room1.bottom_right_y, room2.bottom_right_y),
                           room1.bottom_right_x, min(room1.top_left_y, room2.top_left_y), (room1.id, "E"))
    ...
    def calculate_edge_move(self, room: Room, direction: str, coordinate: str) -> None:
        ...
        if (direction == "E" and coordinate == "R"):
            room_obj.rel_push_R = max(
                room_obj.rel_push_R, 0.5*self.corridor_thickness) if room_obj.rel_push_R >= 0 else room_obj.rel_push_R

        elif (direction == "E" and coordinate == "L"):
            room_obj.rel_push_L = max(
                room_obj.rel_push_L, 0.5*self.corridor_thickness) if room_obj.rel_push_L >= 0 else room_obj.rel_push_L

        elif (direction == "W" and coordinate == "R"):
            room_obj.rel_push_R = min(
                room_obj.rel_push_R, -0.5*self.corridor_thickness) if room_obj.rel_push_R <= 0 else room_obj.rel_push_R

        elif (direction == "W" and coordinate == "L"):
            room_obj.rel_push_L = min(
                room_obj.rel_push_L, -0.5*self.corridor_thickness) if room_obj.rel_push_L <= 0 else room_obj.rel_push_L

        elif (direction == "N" and coordinate == "T"):
            room_obj.rel_push_T = max(
                room_obj.rel_push_T, 0.5*self.corridor_thickness) if room_obj.rel_push_T >= 0 else room_obj.rel_push_T

        elif (direction == "N" and coordinate == "B"):
            room_obj.rel_push_B = max(
                room_obj.rel_push_B, 0.5*self.corridor_thickness) if room_obj.rel_push_B >= 0 else room_obj.rel_push_B

        elif (direction == "S" and coordinate == "T"):
            room_obj.rel_push_T = min(
                room_obj.rel_push_L, -0.5*self.corridor_thickness) if room_obj.rel_push_T <= 0 else room_obj.rel_push_T

        elif (direction == "S" and coordinate == "B"):
            room_obj.rel_push_B = min(
                room_obj.rel_push_B, -0.5*self.corridor_thickness) if room_obj.rel_push_B <= 0 else room_obj.rel_push_B
    ...
    def check_dimensions_feasibility(self) -> bool:
        ...
        for i in range(len(min_width)):
            ...
            if (width < min_width[i] - 2*self.corridor_thickness or height < min_height[i] - 2*self.corridor_thickness or width/height < min_ar[i]-0.2):
                flag = 1
                break
        if flag == 1:
            return False
...
def plot(graph: nx.Graph, m: int) -> None:
    ...
    nx.draw_networkx(graph, pos, label=None, node_size=400, node_color='#4b8bc8', font_size=12,
                     font_color='k', font_family='sans-serif', font_weight='normal', alpha=1, bbox=None, ax=None)
...
def main():
    ...
    def test_circ():
        ...
        plot(circulation_obj.circulation_graph,
             len(circulation_obj.circulation_graph))
